...
| 18e | **Birleştirme ve canlıya uygulama sırası:** 0013 main'e girip canlıya uygulanmadan `DATABASE_URL`li (canlı) bir kapı koşusu 5 katalog testinde KIRMIZIDIR — doğru davranış, ama ilk göreni şaşırtır Bağlantı hatasında pytest'in uzun traceback'i psycopg karesindeki bağlantı dizesini (parola dâhil) loga basıyordu; `verify.sh` ve `scripts/sandbox_db.sh` artık `--tb=short` ile koşar ve `tests/test_gate_traceback.py` bunu sabitler (düzeltme turu 2; `--tb=short` ile ölçülen parola geçişi 0) | Canlıya uygulama controller'da | 0013 canlıya uygulanınca kapanır; sonraki migration'lar için de aynı sıra: önce canlı, sonra `DATABASE_URL`li kapı |
