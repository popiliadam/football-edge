...
| 10r | footystats GitHub runner'larında koşamıyor; iş Mac'te launchd ile koşuyor (R73, RUNBOOK §3.9) | İlk canlı turda 6/6 lig 403 (Cloudflare veri merkezi IP'lerini geri çeviriyor), aynı kod ve kimlik Mac'ten 200. Kör noktalar: Mac o saatte KAPALIYSA tur hiç koşmaz (uykudaysa uyanınca bir kez koşar) ve xG'nin o günkü ara durumu kalıcı kaybolur; Mac günlerce kapalıysa alarm çalmaz, çünkü bekçi yalnız GitHub workflow'larını izler; `gh` oturumu ya da anahtar zinciri yoksa alarm iletilemez; günlük dosyası döndürülmüyor. Önerilen kapanış: yerel iş her turdan sonra boş bir workflow'u `workflow_dispatch` ile tetikler, bekçi onun yaşını öbür tetikler gibi (ör. 72 sa eşik) ölçer |
