...
| 19b | `sources-audit` ve `full-scan` workflow'ları secret taramasını (`scripts/check_secrets.sh`) koşmaz; `sources-audit` `contents: write`, `full-scan` `DATABASE_URL` taşır (T8). Tarama bugün yedi workflow'un ilk adımıdır, "her zamanlanmış workflow"un değil | B-1 dışı, önceden var olan durum | Bu iki workflow'a ilk dokunuşta ya da yeni bir secret'lı workflow eklenmeden önce |
