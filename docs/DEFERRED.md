...
| 18g | **Parolanın test çıktısına sızması.** Kapanan kısım (18e'den taşındı): bağlantı hatasında pytest'in uzun traceback'i psycopg karesindeki bağlantı dizesini (parola dâhil) loga basıyordu; `verify.sh` ve `scripts/sandbox_db.sh` artık `--tb=short` ile koşar ve `tests/test_gate_traceback.py` bunu sabitler (düzeltme turu 2; `--tb=short` ile ölçülen parola geçişi 0); RUNBOOK §2.5'in canlı katalog komutu da `--tb=short` taşır. **Açık kalanlar:** (a) `-l`/`--showlocals` parolayı `--tb=short` ile de yeniden sızdırıyor (`PYTEST_ADDOPTS=-l` ile 85 isabet) ve bekçi testi (`SAFE_TRACEBACK`) bunu görmüyor; `# … --tb=short` satır sonu yorumu da koşu sayılıyor (bütün-dal incelemesi mutasyon B ve C2 yeşil kaldı). Düzeltme: `-l`, `--showlocals`, `--tb=long` ve `--tb=auto` reddi ve satır sonu yorumlarını yok sayma. (b) Bozuk DSN'de libpq traceback biçiminden bağımsız olarak parolayı yankılıyor (`invalid percent-encoded token: "<parola>%zz"`, `missing "=" after "<parola>"`); önceden var olan davranış. Zamanlanmış işlerde (seal/snapshot/…) bu loglar herkese açık Actions loglarıdır ve GitHub yalnız secret'ın tamamını maskeler | (a) bilinçli bir düzenleme ister; (b) canlı DSN bugün geçerli, yalnız secret yanlış yeniden yapıştırılırsa ısırır | (a) `verify.sh`/`sandbox_db.sh`/`test_gate_traceback.py`ye ilk dokunulduğunda; (b) `DATABASE_URL` secret'ı değiştirilmeden ya da yeni bir zamanlanmış DB işi eklenmeden önce |
