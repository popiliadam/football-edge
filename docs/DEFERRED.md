...
| 10k | `seal.yml` push'u yeniden denemiyor ve değişiklik olmasa da push'luyor | Mühür turu sırasında `main`e başka bir commit düşerse (insan push'u, `sources-audit` botu) push reddedilir: yanlış `🔴 seal kırmızı` ve o turun çıpası yayınlanmaz. Çözüm: yalnız commit varsa push + merge'le yeniden deneme (C4 incelemesi; sıradaki iş) |
