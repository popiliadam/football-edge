...
| 11b | Elle yazılmış proxy döndürme (ör. httpx `proxy=` döngüsü) ölçülmüyor; yalnız adı bilinen iki döndürücü (`ProxyRotator`, `requests-ip-rotator`) aranıyor. Docstring'in "ölçülmeyenler" listesinden bu madde düştü. **R77b ile değişti (2026-09-23):** (a) kuralı her çağrıda `proxy=`/`proxies=`/`proxy_auth=`/`proxy_rotator=` anahtar argümanını kırmızı verir; `**{"proxy": ...}` açımı ve httpx `trust_env` hâlâ görünmez (docstring "Bilinen sınırlar"). **`**{"proxy": ...}` açımı 4cbc798'de kapandı** (adı TAM taşıyan dize ve sözlük anahtarı artık kırmızı); httpx `trust_env`/ortam değişkeniyle proxy hâlâ görünmez | yok |
| 11c | R80'in yol listesi `scrapling.spiders`, `core.ai`, `core.shell`, `cli`yi kapsamıyor (hepsi fetcher'lara ulaşır). **R77b ile değişti (2026-09-23):** fetcher'lar serbest; bu dört yol `fetchers`/`engines` ile birlikte (d) tek geçidin `GATED_MODULES`ine girdi — `src/football_edge/scrape.py` dışında kırmızı (kurulu 0.4.15'te ölçüldü). Düzeltme turu 1 (2026-09-23): Scrapling CLI'si (`--solve-cloudflare`, `--proxy`, `extract`/`shell`/`mcp`; `install` serbest), `opts["proxy"]` dizesi ve adı UA olan sabit kırmızıya alındı; inceleme Minor 1–6 (takma adla yeniden ihraç, `sources.yaml`in yalnız `user_agent` alanı, göreli dinamik import, kapalı bot listesi, ortam değişkeniyle proxy ve UA demet listesi, Python'da ekstra taşıyan kurulum dizesi) test docstring'inde bilinen sınır olarak yazılı | `scrapling/engines/static.py` import anında `curl_cffi` istiyor: kilit testi + çalışma zamanı `ImportError` |
...
| 11g | **Scrapling tarayıcı taşıyıcıları (`dynamic`/`stealthy`) kapalı** (oturum 9 Task 3, düzeltme turu 1, controller kararı). Playwright 1.63 `page.route` belgesi: yönlendirmede işleyici yalnız İLK URL için çağrılır ve service worker'ın yakaladığı istekleri görmez; Scrapling bağlamı `service_workers: "allow"` kurar. Yani sıçramalar bekçiden (orijin + iki eksen robots) ve crawl-delay'den geçmeden izlenir. **Ölçülecek:** kurulu gerçek tarayıcıyla (yerel, CI değil) aynı orijinde yönlendiren, başka orijine yönlendiren ve service worker kaydeden yerel sunucu sayfalarına karşı her ağ isteğinin `_browser_fetch` bekçisinden geçtiği (ör. `route.fetch(max_redirects=0)` + `route.fulfill` ile sıçramayı rotaya düşürme, service worker'ları kapatma) ve alt isteklerin crawl-delay/403-429 kurallarına bağlandığı. **Açma koşulu:** bu ölçümler bir testte (tarayıcılı, işaretli) kırmızıya düşürülebilir biçimde kanıtlanır, `build_transport` reddi ve `BrowserTransport.__init__` reddi o testle birlikte kaldırılır | `build_transport` ve `BrowserTransport` `BrowserTransportDisabled` fırlatır (`tests/test_scrape.py`, mutasyonla kırmızı) |
| 11h | **Oturum 9 bütün-dal incelemesinin kayda geçmemiş küçükleri** (2026-09-24). (a) `scrape.py:249` `_carries_credentials`in ters bölü dalı testsiz: `or "\\" in url` silinince `test_scrape.py`+`test_pfdk.py` yeşil kalıyor (bütün-dal incelemesinde mutasyonla ölçüldü); `Location: /\evil.example/x` sıçramasını (WHATWG'de `//evil.example/x`) yalnız bu dal reddediyor. (b) Defter satırı 27, T3 yeniden incelemesi: N10 — `scrape.py:376` önbellekli robots yolundaki `max(state.delay, …)` testsiz (önce `Crawl-delay: 9` alan UA, sonra gecikmesiz UA → hâlâ 9 beklenmeli); N7–N9 — `scrape.py:279` `except`indeki `OverflowError`/`IndexError`/`TypeError` Python 3.11'de hiç atılmıyor (eşdeğer mutant, zararsız savunma; testli sanılmasın). (c) T2 yeniden incelemesinin Minor-B/C/D'si: defter satırı 17'ye göre docstring'e ve 11c'ye yazılmış olmalıydı, yazılmamış. Yanlış pozitifler: düzyazıdaki `--proxy`/`--solve-cloudflare`/`scrapling shell` (hata/log mesajı, yorum) kırmızı olur; savunma amaçlı `{'proxy'}` yasak listesi kırmızı olur. Kaçışlar: `shutil.which("scrapling")` ile kurulan argv başı, kabuk satır devamı (`scrapling \` + `extract …`), demet hedefli UA (`USER_AGENT, N = "Googlebot/2.1", 1`), `scrapling-mcp`. **Ne zaman:** (a) ve (b) herhangi bir Scrapling kaynağı açılmadan ya da zamanlanmadan önce (Plan 2); (c) `tests/test_access_method_rule.py` docstring'ine "Bilinen sınırlar" olarak, dosyaya ilk dokunulduğunda | (a)(b): `tff-pfdk` `enabled: false`, zamanlanmış iş yok, tarayıcı yolu kapalı (11g) — bugün maruziyet yok. (c): yanlış pozitifler kırmızı verir, sessiz değil; kaçışlar bilinçli yazım ister (kazara girişi durdurur) |

## 12. Faz 2 planından ertelenenler (2026-09-22)

| # | Ne | Neden önemli |
|---|---|---|
| 12a | ~~Eski append-only tablolarda (0001–0005: `odds_snapshots`, `source_observations`, `match_results`) TRUNCATE tetikleyicisi ve RLS yok~~ **KAPANDI (2026-09-23, oturum 9 Task 6, `a834ea7`; CANLIYA UYGULANDI 2026-09-23 21:36 UTC, Supabase sürümü `20260923213629`, sha256 `978346eb7ac378c22617047fb86b2d84be5f3b4f5bc4199b9ff5fc228cf182fb` = işlenen dosya; önce ROLLBACK'li kuru koşu, uygulamadan sonra advisors'ta ERROR/WARN yok, ilk mühür turu (21:45, pg_cron dispatch) başarılı, `DATABASE_URL`li kapı 11/11 yeşil):** `0013_api_roles_lockdown.sql` — altı eski tabloda RLS (FORCE yok, politika yok); `public`teki bütün tablo/dizi/fonksiyonlardan `anon`/`authenticated` yetkileri ve `for role postgres` varsayılan yetkileri geri alındı (+ `postgres`in yeni fonksiyonlarında PUBLIC EXECUTE); üçlüye TRUNCATE bekçisi; `forbid_ledger_mutation` `search_path = ''` ve `tg_table_name` ile. Test `tests/test_api_roles_lockdown_db.py` (katalog: `DATABASE_URL`; kum havuzu: `SANDBOX_DATABASE_URL`, supabase/postgres 17.6). Bilinen sınır: `supabase_admin`in varsayılan yetkileri `postgres`ten değiştirilemez | Satır tetikleyicisi UPDATE/DELETE'i durdurur, TRUNCATE'i durdurmaz; RLS'siz tablo, Supabase'in API rolleri yetkiliyse REST'ten okunabilir. 0006/0007 ikisini de açıyor (R90) — eski tablolar ayrı bir migration ister (Supabase advisors ile birlikte bakılmalı). Aynı migration 0001'in tetikleyici mesajını da düzeltmeli: her tabloda "odds_snapshots" diyor (`tg_table_name` kullanılmalı; Task 3 incelemesi) |
...
| 16e | **Gölge bayatlık yarıçapı:** eşlenemeyen tek fikstür o ülke grubunu 10 gün bayat yapar (`is_stale` `None` anahtarı eksik sayar); `pre_prices` son tur eksikse önceki tura düşmez | Tasarım §7.3 muhafazakârlığı | Faz 4 gölge CLV raporu: yalnız eşlenmiş fikstürleri yargılamak mı, takma ad hijyeni mi — ölçüp karar · **Takma ad hijyeni 2026-09-23** (oturum 8, `f640fa1` öncesi `6ca8547`): salt okuma teşhisiyle 51 canlı maçta tek eşlenmeyen (`Erzurum BB` → `Erzurumspor`, eski sezon adına düşüyordu); `live parity` 51/0/0/0. Teşhis betiği (salt okuma, `DATABASE_URL` ister): `.superpowers/sdd/2026-09-23-kucuk-borc-temizligi/aliases_diag.py` — `PYTHONPATH` gerekmez, depo kökünden `uv run python <yol>`; `SINCE` sabitini güncelle. Bayatlık yarıçapı kararı (yalnız eşlenmiş fikstür mü) hâlâ 17j ile, ilk dolu raporda |
...
| 16g | **B1:** DC memo'su aynı nesne başka maç kümesine oynatılırsa fitleri taşır (üretimde tetiklenmiyor, AST ile kanıtlı) | Gizil | Memo anahtarına gruptaki gözlem sayısı; `model/strategies.py`nin ilk değişikliği · **2026-09-23 değerlendirildi, bekliyor:** düz "gözlem sayısı" anahtarı `cadence_days > 1`de aynı fit günü içinde her yeni gözlemde yeniden fit tetikler (fit yalnız `at`ten önceki maçları okur → sonuç aynı, maliyet katlanır); doğru anahtar "`at`ten önceki gözlem sayısı", ucuz hesabı sıralı parça yapısı ister — ayrı tasarım |
...
| 16j | Kod tekrarları: `_gap` (wf_eval, model_selftest), `_unmeasured` (selftest, model_selftest), üç aralık biçimleyici, `"Avg"` sabiti ×4, `_LOG_FLOOR` ×4; `kinds_of`/`rating_groups` bir CLI modülünde (`live/__main__` oradan alır) | Plan metni aynen yazıldı | `kinds_of`/`rating_groups` → `history/catalog.py`; tek `format_interval`, tek kitap sabiti · **Kapandı 2026-09-23** (plan `2026-09-23-kucuk-borc-temizligi`, T1–T3): `REFERENCE_BOOK` (`history/types.py`) ve `LOG_FLOOR` (`market/metrics.py`) tek kaynak — AST bekçisi `tests/test_single_sources.py`; `interval_text`, `log_loss_gap`, `unmeasured`; `kinds_of`/`rating_groups` `history/catalog.py`de. `live/context.REFERENCE_BOOK` bilinçli ayrı (canlı kitap) |
| 16k | `frozen_weights` hedeflerinde ek lig anahtarları (anlamsız `fallback` girdileri); `EloModelConfig.draw` `ordered` biçimde ölü alan; `EloModelConfig` k/ha/gap/initial doğrulanmıyor | Zararsız | Yapılandırma okuyucusu sağlamlaştırılırken · **(c) kapandı 2026-09-23** (plan `2026-09-23-kucuk-borc-temizligi`, T6: K sonlu ≥ 0, ev avantajı sonlu, başlangıç sonlu > 0, sezon arası tam gün ≥ 1). (a) = 17d, Plan 2 T10. (b) mühürlü `model_faz3.yaml` `ordered`da `draw: 0.26` taşıyor — reddetmek mühürlü dosyayı kırar; bir sonraki model dosyası `draw`sız yazılabilsin diye alan isteğe bağlı yapılmalı · **(b) Kapandı 2026-09-23** (oturum 9, `bb3a360`): `load_model_config` `draw_form: ordered` bölümünde `draw`ı isteğe bağlı okur (eksikse sınıf varsayılanı; olasılığa girmez), `quadratic`te ve biçimi yazmayan dosyada zorunlu kalır; yazılan `draw` yine [0, 0,5] denetlenir. Mühürlü `model_faz3.yaml` değişmedi, aynen okunur ve baytları ön kaydın `model_config_sha256`sına bağlı (test). Bilinen sınır: `dump_model_config` (`select`) `ordered`da `draw`ı hâlâ yazar; alan `None` alabilir yapılmadı — `selection.elo_loss`un `replace(config, draw_form=QUADRATIC)` yeniden oynatması δ'suz yapılandırmayı reddederdi |
...
| # | Ne | Neden bekliyor | Ne zaman bakılır |
|---|---|---|---|
| 17a | **`jev_item_answers` okuyucu mühürü:** tablodaki `t1_failed:<n>` işaret satırlarını her SELECT süzmeli ya da `gates_from`a vermeli (son inceleme I-3) | Bugün tek okuyucu `tier1.py` ve süzüyor | **Plan 2 T5'in ilk adımı**, tabloya yeni okuyucu yazılmadan önce test · **Kapandı 2026-09-23** (oturum 8, `f640fa1`): `tests/test_jev_item_answers_readers.py` — src'deki her SELECT+`jev_item_answers` metni adı yazılı okuyucu; okuyucu başına süzgeç YÖNÜ sabit (`_ASKED` NOT, `_ATTEMPTS` düz). Bilinen kaçışlar 17h'ye eklendi |
| 17b | Kesinti koşusu: bütün haberler `jev_error` ile düşerse işaret yazılmamalı (3 ardışık kesinti = o `prompt_version` için kalıcı kapsam kaybı) | `tier1` zamanlanmadı | Plan 2 T4, zamanlamadan önce · **Kapandı 2026-09-23** (oturum 8, `f640fa1`): kesinti = art arda `OUTAGE_STREAK` (3) Jev hatası — koşu durur (≤ 3 çağrı), serinin işareti yazılmaz, önceki cevaplar korunur; seriye ulaşmayan hata eskisi gibi işaretlenir. CLI `EXIT_SOURCE_FAILED` (7). **`tier1` zamanlanırken:** workflow adımı 7'yi "Jev kesintisi" diye adlandırmalı (sync-news'te 7 = kaynak) |
...
| 17g | Gölgede reddedilen karar fiyatı (Σ1/o<1) sayılmaz (`live/shadow.py`); raporda `incomplete`e karışır | Ölçülen 1X2/pre red 0 | Gölge dosyasının ilk değişikliği · **Kapandı 2026-09-23** (oturum 8, `f640fa1`): gölge logu `reddedilen fiyat N`; rapor `fiyatı reddedilen M` (yalnız MARKET eksik ve saklı `pre` reddediliyorsa); `shadow_rows` satırları bayt bayt aynı |
| 17h | AST bekçilerinin bilinen kaçışları: import kuralında yerel adla yeniden bağlama / `getattr`; bütçe bekçisinde `TypeSafeJev as X` takma adı ve sarmadan önce çıplak kullanım | Kazara girişi durdurur, bilinçli kaçışı değil (Faz 3 §3.1/12) | Kırmızı takım (Plan 2 T10) · **2026-09-23 eklenenler (17a bekçisi, son inceleme M-4/M-5):** tablo adı sabitiyle kurulan f-string (`f"… FROM {TABLE}"`), `%`/`.format`/`psycopg.sql.Identifier`, `+` birleştirme, büyük harf tablo adı, `db/migrations` VIEW/fonksiyon ve `scripts/` taranmaz; "`gates_from`a ver" dalı listeye yazılamıyor (ayrı `GATES_READERS` kümesi gerekir) · **17a bekçisi kaçışları kapandı 2026-09-23** (oturum 9, `bb3a360`): metin statik kurulur — aynı modülün düz metin sabitiyle f-string, `+`, `%` (tek/demet), `.format` (konumlu/adlı), `psycopg.sql.SQL/Identifier/Literal` ve büyük harf tablo adı yakalanır (sentetik ağaçta her biri adıyla kırmızı); `GATES_READERS` ayrı küme (bugün boş) ve sabitinin her kullanımı `gates_from` çağıran fonksiyonda olmalı (`misrouted`, sentetik ağaçta iki yöne de sınandı). Bilinen sınır (docstring'de): başka modülden import edilen ya da nitelikle okunan sabit, fonksiyon içinde kurulan ad, `db/migrations` VIEW/fonksiyon ve `scripts/` taranmaz; `gates_from`a verme yalnız aynı fonksiyon gövdesinde aranır. İlk satırın import/bütçe bekçisi kaçışları açık · **2026-09-24 eklenenler (T4 incelemesi, bütün-dal triyajı; docstring'de yazılı değil):** 17a bekçisi kaçışları — `%`-dict (`"… %(t)s" % {"t": TABLO}`), `TABLO.upper()`, birleştirilmiş sabit (`TABLO = "jev_" + "item_answers"`), `" ".join((…, TABLO))`; `obj.gates_from(...)` çağrı biçimi testsiz (dal silinince 16 test yeşil; `GATES_READERS` bugün boş, dalın kullanıcısı yok) |
| 17i | `_budgeted` iki kopya (`collect.py`, `features/__main__.py`); `LockViolation→9` bloğunun üçüncü kopyası; `_calibration` dışında `backtest/evaluate.py:85` çıplak `calibration()` | Bekçi modül başına sarmalayıcı tanır | 16j ile birlikte · **Kapandı 2026-09-23** (plan `2026-09-23-kucuk-borc-temizligi`, T2/T4/T5): `jev_budget.budgeted_jev` (bekçi bütçe modülünün sarmalayıcılarını tanır, sarmayanı tanımaz — mutasyonla kanıtlı); `lock.refuse_on_violation` + tek `EXIT_LOCK_VIOLATION` (altı çağrı yerinin mesajı birebir testle sabit); `evaluate` `calibration_or_none`; `model_selftest` W4 yalnız `CalibrationUnfit`i "ölçülemedi" sayar, biçim hatası yükselir |
...
| 17l | ajansspor gövdesi robots'a göre okunabilir ama ToS okunmadı | Sinyal şimdilik başlık | Plan 2'de gövde toplamadan önce · **2026-09-23 araştırıldı** (`docs/reports/2026-09-23-kaynak-kosullari.md`): sözleşme robots'ta kapalı, okunabilen yasal sayfalarda yasak yok, robots `Content-Signal: ai-input=yes, ai-train=no` — kapatma ya da yayıncıya sorma kullanıcı kararı |
| 17m | Küçükler (hepsi "kalır"): ağırlık dosyasında `_vector` bool, `version: true`; rapor metninde sabit yol; `test_live_report.py` 552 / `test_feature_tier1.py` 632 satır; `forbid_ledger_mutation()` mesajı `odds_snapshots` adını taşır; `shift` olasılık uzunluğu doğrulanmaz; `_words` karesel; yakın eşdoğrusal soru çiftleri (T9 budaması); `QUESTIONS_PATH` CWD-göreli | Düşük değer | Dosyaya dokunulduğunda · **Kısmen kapandı 2026-09-23** (plan `2026-09-23-kucuk-borc-temizligi`, T6): `version: true` iki okuyucuda da red; `shift` 3'lü olmayan vektörü reddeder; `_vector` bool zaten reddediliyordu (`live/weights.py`, `5d9c4a2`). Kalanlar: `QUESTIONS_PATH` projedeki bütün CLI varsayılanları gibi CWD-göreli (tek başına değiştirmek tutarsızlık); ~~`forbid_ledger_mutation()` mesajı migration ister~~ **0013 ile kapandı** (12a: `tg_table_name` + `search_path = ''`, `test_0013_trigger_function_names_the_table_and_pins_search_path` sabitler); `_words` ve eşdoğrusal sorular Plan 2 tier1; test dosyası boyutu, rapordaki sabit yol |
| 17n | **Boş tur bekçisi (exit 19, oturum 8) küçükleri:** olay tam ufukta (`<=`) ve eksik alanlı / liste olmayan `/events` yükü (KeyError/TypeError dalı; bozuk saat testli) testle sabit değil; gölge logunda `config.method` yerine sabit yöntem mutantı yaşıyor (fikstür POWER) — son inceleme M-3 | Davranış doğru, test zayıf | Bekçi ya da gölge dosyasının ilk değişikliği · **Kapandı 2026-09-23** (oturum 9, `bb3a360`): `tests/test_collect_empty_round.py` — tam `commenceTimeTo` anındaki fikstür exit 19 verir (`<` mutantı kırmızı); `commence_time`sız, liste olmayan ve `null` `/events` yükü uyarıyla exit 0 (KeyError/TypeError dalı, hata türü logda). `tests/test_live_cli.py` — Σ 1/o = 2 fiyatı power temizler, Shin reddeder: SHIN yapılandırmasında gölge satırı `yazılan 11 · reddedilen 1` (logda ve `shadow_rows`ta sabit `power` mutantları kırmızı) |

## 18. Oturum 9 Task 6'dan (0013, API rolleri kilidi) ertelenenler (2026-09-24)

Kaynak: Task 6 incelemesi (`.superpowers/sdd/2026-09-23-oturum9-dalga-a/task-6-review.md`, gitignored).

| # | Ne | Neden bekliyor | Ne zaman bakılır |
|---|---|---|---|
| 18a | **KISMEN KAPANDI (Faz 6 B-1 Task 9):** CI'da iş içi kap — kum havuzu ve `sitedb` testleri koşar; `DATABASE_URL`li katalog testleri SKIP kalır. ~~**CI kum havuzu DB testlerini koşmuyor.**~~ `tests/test_api_roles_lockdown_db.py`nin 15 DB testinden kum havuzu katmanının 10'u (sahibin yazımı, API rollerinin reddi, TRUNCATE bekçisi, idempotentlik, kilit sınırı) artık CI'da koşar ve `CI=true` iken değişken yoksa atlanmaz, kırmızı verir; katalog katmanının 5'i kapıda ve CI'da adıyla SKIP, kanıtı yalnız yerelde `scripts/sandbox_db.sh test` (RUNBOOK §4). CI kabı secret istemez (parola iş içinde üretilir) | Açık kalan yalnız katalog katmanı: `DATABASE_URL` ister ve CI'a verilmez (canlı defter). ~~Hizmet kabı ~1,7 GB imaj çeker ve her push'a süre ekler; kapının sayım düzeni (`EXPECTED_MIN_*`) DB testlerini henüz saymıyor~~ — kap eklendi; `sitedb` testleri `EXPECTED_MIN_SITEDB` ile sayılır, kum havuzu testleri sayılmaz ama `CI=true` altında atlanamaz | Katalog katmanı için: CI'da migration'ları commit'li uygulanmış ikinci bir iş içi kap (yerel `applied` kabının eşi) ya da ilk "yerelde yeşil, CI'da hiç koşmadı" olayı |
| 18b | **Sahip append-only bekçileri aşabilir:** `postgres` `set session_replication_role = replica` ile ya da `alter table … disable trigger` ile TRUNCATE/UPDATE/DELETE yapabilir (incelemede ölçüldü; `anon`/`service_role` parametreyi değiştiremez). On append-only tablonun hepsi için geçerli; API rolü yolu değil | Sahip hattın kendisi; Supabase'de `postgres`in bu yetkisi kısılamaz. Karşı önlem ayrı bir yazar rolü (sahip olmayan, yalnız INSERT) + olay tetikleyicisiyle `disable trigger` alarmı ister | Yazar rolü ayrımı yapılırken (RUNBOOK §2.3 "DISABLE TRIGGER YASAK" bugün yalnız yazılı kural) |
| 18c | ~~RUNBOOK §2.5 kesiti yeni defteri indekssiz kuruyordu~~ **KAPANDI (düzeltme turu 1):** yeniden adlandırılan tablo `odds_match_idx`/`odds_closing_idx` adlarını tutuyordu, 0001'in `if not exists`i sessizce atlıyordu. §2.5 adım 1 artık pkey, iki indeks ve diziyi de taşıyor; yerel kapta geri alınan işlemde dört indeks özgün adlarıyla yeniden kuruldu | — | — |
| 18d | Kum havuzu fikstürü bütün migration'ları TEK işlemde uygular: işlem içinde koşamayan bir ifade (`create index concurrently`, `vacuum`, `alter type … add value` kullanımı) kum havuzu katmanını ERROR'a çevirir | Bugün yok | Böyle bir ifade taşıyan ilk migration'da: fikstür o dosyayı ayrı, commit'li bir veritabanında uygulamalı |
| 18e | ~~**Birleştirme ve canlıya uygulama sırası:** 0013 main'e girip canlıya uygulanmadan `DATABASE_URL`li (canlı) bir kapı koşusu 5 katalog testinde KIRMIZIDIR — doğru davranış, ama ilk göreni şaşırtır.~~ **KAPANDI (2026-09-23):** 0013 canlıda (12a), `DATABASE_URL`li kapı 11/11 yeşil. Kalıcı kural: sonraki migration'lar için de aynı sıra — önce canlı, sonra `DATABASE_URL`li kapı. (`--tb=short` notu 18g'ye taşındı) | — | — |
| 18f | 0013'ün `set local lock_timeout` yorumu ("migration işlem içinde uygulanır") ölçülmemiş bir varsayım: yeniden incelemede her ifadeyi ayrı ayrı autocommit'te gönderen bir koşucuda (D biçimi) sınır bağlamadı ve kısmi commit oldu (`leagues`+`matches` RLS). İşlem açan üç biçimde (açık BEGIN, tek sorgu, tek Sync'li boru hattı) 5 sn'de düştü. 0013 canlıya uygulanırken değiştirilmedi | `apply_migration`ın gönderim biçimi kapalı kaynak; D biçimi atomikliği de bozar ve bunu hiçbir `lock_timeout` biçimi düzeltmez | Bir sonraki kilit alan migration'da `set lock_timeout = '5s'` … `reset lock_timeout` biçimi (dört biçimde de bağlar) ve uygulama sonrası salt okuma doğrulaması |
| 18g | **Parolanın test çıktısına sızması.** Kapanan kısım (18e'den taşındı): bağlantı hatasında pytest'in uzun traceback'i psycopg karesindeki bağlantı dizesini (parola dâhil) loga basıyordu; `verify.sh` ve `scripts/sandbox_db.sh` artık `--tb=short` ile koşar ve `tests/test_gate_traceback.py` bunu sabitler (düzeltme turu 2; `--tb=short` ile ölçülen parola geçişi 0); RUNBOOK §2.5'in canlı katalog komutu da `--tb=short` taşır. (a) **KAPANDI (Faz 6 B-1 son inceleme m2):** bekçi artık `-l`, `--showlocals`, `-ql` gibi kümeleri ve `--tb=long`/`--tb=auto`yu reddeder, satır sonu yorumunu koşu saymaz (beş mutasyon kırmızı; eski bekçide beşi de yeşildi). Ölçmediği: çağıranın `PYTEST_ADDOPTS`u ve `pyproject` `addopts`u (`PYTEST_ADDOPTS=-l` ile 85 isabet ölçülmüştü). ~~`-l`/`--showlocals` parolayı `--tb=short` ile de yeniden sızdırıyor ve bekçi testi (`SAFE_TRACEBACK`) bunu görmüyor; `# … --tb=short` satır sonu yorumu da koşu sayılıyor.~~ **Açık kalan:** (b) Bozuk DSN'de libpq traceback biçiminden bağımsız olarak parolayı yankılıyor (`invalid percent-encoded token: "<parola>%zz"`, `missing "=" after "<parola>"`); önceden var olan davranış. Zamanlanmış işlerde (seal/snapshot/…) bu loglar herkese açık Actions loglarıdır ve GitHub yalnız secret'ın tamamını maskeler | (b) canlı DSN bugün geçerli, yalnız secret yanlış yeniden yapıştırılırsa ısırır | (b) `DATABASE_URL` secret'ı değiştirilmeden ya da yeni bir zamanlanmış DB işi eklenmeden önce |

## 19. Faz 6 İz B B-1 bütün-dal son incelemesinden ertelenenler (2026-09-24)

Kaynak: B-1 son incelemesi (`.superpowers/sdd/2026-09-24-faz6-iz-b-1-okuma-katmani/final-review.md`, gitignored).

| # | Ne | Neden bekliyor | Ne zaman bakılır |
|---|---|---|---|
| 19a | `site/__main__._export` yalnız `ExportRefused` ve `psycopg.Error` yakalar: `devig_method`un (yapılandırma okuma) ya da `_schema()`nin `OSError`u ve `snapshot_errors`/`derive`den gelen beklenmeyen başka bir sınıf, adlı 20–24 kodu yerine traceback'le exit 1 verir (son inceleme m5). Değer sızmaz: varsayılan traceback yerel değişken basmaz, mesajlar denetlendi | Kırmızı kalır (exit ≠ 0, yayın yok); yalnız çıkış kodu adsızdır | `site.yml` ilk kez secret'la koşmadan önce ya da exit koduna dayanan bir adım eklenirken |
| 19b | `sources-audit` ve `full-scan` workflow'ları secret taramasını (`scripts/check_secrets.sh`) koşmaz; `sources-audit` `contents: write`, `full-scan` `DATABASE_URL` taşır (T8). Tarama bugün yedi workflow'un ilk adımıdır, "her zamanlanmış workflow"un değil. **Seal işi `continue-on-error`:** `seal` işine İŞ düzeyinde `continue-on-error: true` eklenirse tarama testi yeşil kalır ve koşu yeşil görünür (B-1 T8 yeniden inceleme 2, p3); kırmızı bir taramadan sonra iki adım yine koşar: `Mühür turunun sonucunu yansıt` (`always()`) ve `Bekçi` (`!cancelled()`, schedule'da `ODDS_API_KEY` alır). `Alarm kapat`ın (`seal.yml:152`) ADIM düzeyi `continue-on-error: true`su kasıtlıdır ve bu satırın konusu değildir | B-1 dışı, önceden var olan durum; bugün iş düzeyinde `continue-on-error` yok | Bu iki workflow'a ilk dokunuşta ya da yeni bir secret'lı workflow eklenmeden önce |
| 19c | `tests/test_site_gate.py::test_ci_never_prints_the_container_log` `docker inspect` satırında yalnız `-f`nin varlığına bakar: `inspect -f '{{json .Config.Env}}'` (kap parolasını basar) bekçiden geçer (T9 n2). Bugünkü `ci.yml` biçimi güvenlidir | Bugün böyle bir satır yok | `ci.yml`in site-db adımına ilk dokunuşta: `-f` biçiminde `.Config` reddi (tek regex) |

## 20. Faz 6 İz B B-2 bütün-dal son incelemesinden ertelenenler (2026-09-24)

Kaynak: B-2 son incelemesinin triyajı (`.superpowers/sdd/2026-09-24-faz6-iz-b-2-web-yuzeyi/final-review.md`,
gitignored) ve görev incelemeleri. Kapının ölçmedikleri ayrıca `docs/phases/06-site/HANDOFF.md` "Kapının ÖLÇMEDİKLERİ
(B-2)"dedir; yasal taslakların avukat soruları aynı dosyanın "Hukuk incelemesi" bölümündedir.

| # | Ne | Neden bekliyor | Ne zaman bakılır |
|---|---|---|---|
| 20a | `web/vitest.config.ts`e `dir`/`root` eklenirse vitest `include` kalıpları başka bir kökten çözülür ve Python bekçisi (`tests/test_site_web_deps.py`, pnpm-10 bekçisinin kapsamda kalması) yeşil kalır (T1) | `.skip` ile aynı sınıf; bugün böyle bir ayar yok | `vitest.config.ts`e ilk dokunuşta |
| 20b | en `record.emptyExplain` sabit değil: yalnız tr cümlesi spec §6.2'de ve `dict.test.ts`te; en çevirisi "will soon beat…" gibi bir vaade dönebilir ("soon" yasak listede değil, "coming soon" var) (T3 m3) | Spec yalnız tr metni veriyor; çeviri kapı dışı | AK2 metin onayında en cümlesi de sabitlenir |
| 20c | tr negatif yüzde `%-4,76` basılıyor; Türkçe tipografi ve `Intl.NumberFormat("tr", {style: "percent"})` `-%4,76` verir (T3 m4). Plan bu biçimi `format.test.ts`te sabitliyor, tarayıcı aynı fonksiyonu çağırıyor | Yalnız görünüm; bugün yalnız boş sicilin CLV'sinde ortaya çıkar | Faz 5 sicili doldurmadan önce Türkçe biçim kararı |
| 20d | `matchLd`in `eventStatus` sınırı (`commence_time == generated_at`) testsiz: `>` → `>=` mutasyonu yeşil, fixture'da eşit anlı maç yok (T4 m5). Hazır test (inceleme sondasından, `web/src/lib/jsonld.test.ts`e): `for (const m of snapshot.matches) { const league = snapshot.leagues.find((each) => each.id === m.league_id); if (!league) throw new Error("lig"); expect("eventStatus" in matchLd({ ...m, commence_time: snapshot.generated_at }, league, matchPath("en", league, m), snapshot.generated_at)).toBe(false); }` | Kod spec'e uygun (`>`); T9 `checkJsonLd` aynı karşılaştırmayı bağımsız yazıyor | `jsonld.ts`e ilk dokunuşta |
| 20e | `LocalTime` saat dilimi adını göstermiyor; `utcLabel` testsiz (T5 m1) | Arayüz metni | Arayüz metinleri onaylanırken (AK2) |
| 20f | Erişilebilirlik: iki `<nav>` aynı erişilebilir adı taşıyor (T5 m2); Next'in 404 sayfalarında `<html lang>` yok (T5 m3) | Plan (g) "tam erişilebilirlik (axe)" sınırıyla aynı sınıf | Erişilebilirlik denetimi (axe) eklenirken |
