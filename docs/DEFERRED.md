...
| 18e | **Birleştirme ve canlıya uygulama sırası:** 0013 main'e girip canlıya uygulanmadan `DATABASE_URL`li (canlı) bir kapı koşusu 5 katalog testinde KIRMIZIDIR — doğru davranış, ama ilk göreni şaşırtır | Canlıya uygulama controller'da | 0013 canlıya uygulanınca kapanır; sonraki migration'lar için de aynı sıra: önce canlı, sonra `DATABASE_URL`li kapı |
