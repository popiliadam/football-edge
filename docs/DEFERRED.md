...
| 17i | `_budgeted` iki kopya (`collect.py`, `features/__main__.py`); `LockViolation→9` bloğunun üçüncü kopyası; `_calibration` dışında `backtest/evaluate.py:85` çıplak `calibration()` | Bekçi modül başına sarmalayıcı tanır | 16j ile birlikte · **Kapandı 2026-09-23** (plan `2026-09-23-kucuk-borc-temizligi`, T2/T4/T5): `jev_budget.budgeted_jev` (bekçi bütçe modülünün sarmalayıcılarını tanır, sarmayanı tanımaz — mutasyonla kanıtlı); `lock.refuse_on_violation` + tek `EXIT_LOCK_VIOLATION` (`backtest._locked_matches` bloğu biçimi farklı olduğu için kaldı); `evaluate` `calibration_or_none` |
