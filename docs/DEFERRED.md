...
| 17n | **Boş tur bekçisi (exit 19, oturum 8) küçükleri:** olay tam ufukta (`<=`) ve eksik alanlı / liste olmayan `/events` yükü (KeyError/TypeError dalı; bozuk saat testli) testle sabit değil; gölge logunda `config.method` yerine sabit yöntem mutantı yaşıyor (fikstür POWER) — son inceleme M-3 | Davranış doğru, test zayıf | Bekçi ya da gölge dosyasının ilk değişikliği |
