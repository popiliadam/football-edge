...
| # | Ne | Neden bekliyor | Ne zaman bakılır |
|---|---|---|---|
| 18a | **KISMEN KAPANDI (Faz 6 B-1 Task 9):** CI'da iş içi kap — kum havuzu ve `sitedb` testleri koşar; `DATABASE_URL`li katalog testleri SKIP kalır. **CI kum havuzu DB testlerini koşmuyor.** `tests/test_api_roles_lockdown_db.py`nin 15 DB testi (katalog + kum havuzu; sahibin yazımı, API rollerinin reddi, TRUNCATE bekçisi, idempotentlik, kilit sınırı) kapıda ve CI'da adıyla SKIP; kanıt yalnız yerelde `scripts/sandbox_db.sh test` (RUNBOOK §4). CI'da supabase/postgres hizmet kabı secret istemez (parola iş içinde üretilir) | Hizmet kabı ~1,7 GB imaj çeker ve her push'a süre ekler; kapının sayım düzeni (`EXPECTED_MIN_*`) DB testlerini henüz saymıyor | Bir sonraki DB migration'ı ya da ilk "yerelde yeşil, CI'da hiç koşmadı" olayı |
