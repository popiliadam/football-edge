...
- **Kapı:** `TMPDIR=$(mktemp -d) ./verify.sh > <log> 2>&1`, sonuç LOG DOSYASINDAN; 10 PASS + `SKIP: zincir` adıyla
  (DB bağlıyken 11/11). **Faz 6 B-2 T10'dan beri** önce `source ~/.nvm/nvm.sh && nvm use 24.21.0` (PATH'teki Node 22
  ile `FAIL: site-kurulum` — `HATA: Node 22, .nvmrc 24 istiyor`); yerelde DB'siz 16 PASS + `SKIP: site-db`,
  `SKIP: site-derleme/e2e`, `SKIP: zincir`; kum havuzu kabıyla (`SITE_TEST_DATABASE_URL`) 17 PASS + `SKIP: zincir`
  (kipler: `docs/phases/06-site/HANDOFF.md` "B-2 T10"). Her commit'ten sonra tam kapı; `main`e `--no-ff`; push öncesi `git fetch && git merge --no-ff
